...
class ActorNet(nn.Module):
    ...
    def __init__(self, num_inputs, hidden1_size, hidden2_size, num_outputs):
        ...
        self.attention = MultiHeadAttention(num_inputs)
        self.conv = nn.Conv1d(2, 1, 1)
        self.flatten = nn.Flatten(start_dim=0)
        self.fc1 = nn.Linear(num_inputs, hidden1_size, bias=False)
        self.fc2 = nn.Linear(hidden1_size, hidden2_size, bias=False)
        self.fc3 = nn.Linear(hidden2_size, num_outputs)

    def forward(self, x):
        x = torch.reshape(x, (-1, 2, 1))
        x = self.attention(x, x, x)
        x = F.relu(self.fc1(self.flatten(self.conv(x))))
        x = F.relu(self.fc2(x))
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        attention = self.attention(q, k, v).permute(0, 2, 1)
